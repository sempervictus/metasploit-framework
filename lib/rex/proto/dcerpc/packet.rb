...
require 'rex/text'
	...
	def self.make_auth(last_response, opts, ntlm_options, auth_type, auth_level)
	...
	def self.make_bind_auth(uuid, vers, domain, name, ntlm_options, auth_type, auth_level)
	...
	def self.make_alter_context_auth(uuid,vers,last_response, opts, ntlm_options, auth_type, auth_level)
                # Process the version strings ("1.0", 1.0, "1", 1)
                bind_vers_maj, bind_vers_min = UUID.vers_to_nums(vers)
                xfer_vers_maj, xfer_vers_min = UUID.vers_to_nums(UUID.xfer_syntax_vers)
	...
	def self.make_request_chunk_auth(flags=3, opnum=0, data="", ctx=0, object_id = '', auth_type=nil, auth_level=nil)
		...
		auth << Rex::Proto::DCERPC::Authentication.auth_buff(auth_type, auth_level, 12)
		...
		flen += auth.length 
                ...
                [
                        ...
                        ctx,    # context id
                        opnum,  # operation number
                ].pack('CCCCNvvVVvv') + object_str + data + auth + Rex::Proto::DCERPC::Authentication.ntlmssp_verifier()
        end


	def self.make_request_auth(opnum=0, data="", size=data.length, ctx=0, object_id='', auth_type=nil, auth_level=nil)
		return [make_request_chunk_auth(0x83, opnum, data, ctx, object_id, auth_type, auth_level)]
	end
