...
module Powershell
  ...
  def initialize(info = {})
    ...
    register_advanced_options(
      [
        OptInt.new('Powershell::timeout',   [true, 'Powershell execution timeout, set < 0 to run async without termination', 30]),
        OptBool.new('Powershell::log_output', [true, 'Write output to log file', false]),
        OptBool.new('Powershell::dry_run', [true, 'Write output to log file', false]),
      ], self.class)
  end
  ...
  def execute_script(script, greedy_kill = false)
    ...
    session.response_timeout = datastore['Powershell::timeout'].to_i
    ps_bin = datastore['Powershell::RUN_WOW64'] ? '%windir%\syswow64\WindowsPowerShell\v1.0\powershell.exe' : 'powershell.exe'
  ...
  def get_ps_output(cmd_out, eof, read_wait = 5)
    ...
    if datastore['Powershell::log_output']
  ...
  def psh_exec(script, greedy_kill=true, ps_cleanup=true)
    ...
    env_suffix = Rex::Text.rand_text_alpha(8)
    script = PowershellScript.new(script) unless script.respond_to?(:compress_code)
    ...
    if datastore['Powershell::dry_run']
      ...
      if datastore['Powershell::timeout'].to_i < 0
        out =  "Started async execution of #{running_pids.join(', ')}, output collection and cleanup will not be performed"
        # print_error out
        return out
      end
      ps_output = get_ps_output(cmd_out,eof,datastore['Powershell::timeout'])
